...
1. **Issue Creation (`create-issue`)**:
   - Every task, documentation addition, refactoring, or feature MUST start with an issue file under the `docs/issues/` directory.
   - Use the `create-issue` skill to initialize a new issue and register it in [docs/issues/README.md](../docs/issues/README.md).
...
2. **Issue Refinement (`polish-issue`)**:
   ...
   - Update the issue status to `In Progress` in both the issue header and [docs/issues/README.md](../docs/issues/README.md).
...
4. **Issue Closing & Git Workflow (`git-workflow`)**:
   ...
   - Move the issue file to `docs/issues/closed/<issue-id>-<desc>.md`.
   - Update the status in [docs/issues/README.md](../docs/issues/README.md) to record the issue under "2. 完了済み Issue 一覧".
...
- **IPA Syllabus & Official Terminology Compliance**:
  - All exam learning materials in `src/content/` MUST strictly comply with official IPA terminology (Syllabus Ver.2.1 and Subject A-2 Supplement Ver.4.0).
...
- Threat modeling results MUST be outputted under the `docs/` directory when analyzing system security features.
- For any identified vulnerabilities or threats addressed, include links to corresponding closed issue files under `docs/issues/closed/`.
