...
- **概要**: 組織が情報資産の機密性・完全性・可用性（CIA）をバランスよく維持・管理し、リスクを適切に制御するための情報セキュリティマネジメントシステムに関する国際・日本産業規格。
- **技術・運用ポイント**: PDCAサイクル（Plan:リスクアセスメント・方針策定, Do:管理策の適用, Check:内部監査・レビュー, Act:是正処置）の継続的改善プロセスを運用する。
- **試験出題ポイント**: 管理策（附属書A / JIS Q 27002）の適用選定や「適用宣言書 (SoA)」の作成要件が午前II・午後記述で出題。
- **シラバス参照**: [1-1 情報セキュリティ方針の策定](../syllabus_detail.md)
...
- **概要**: 経済産業省が策定した、情報セキュリティ監査の品質向上と実施者の客観性・独立性を確保するための基準（監査人の適格性、監査計画、実施手順等を提示）。
- **技術・運用ポイント**: 「一般基準」「実施基準」「報告基準」の3部構成。
- **試験出題ポイント**: システム監査基準との関係や監査人の独立性要件（外観上の独立性と精神上の独立性）。
...
- **概要**: 経済産業省が定めた、ITシステム全般のガバナンス、安全対策、有効性、効率性を独立した立場で検証・評価するための基準。
- **技術・運用ポイント**: 主に情報システムの「可用性・信頼性・安全度」の評価と助言・保証を行う。
- **試験出題ポイント**: 監査人の立ち位置（ラインからの独立）と経営陣への報告義務。
- **シラバス参照**: [1-5 情報セキュリティ監査](../syllabus_detail.md)
...
- **概要**: 組織の枠を超えて CSIRT 同士（NCA: 日本CSIRT協議会や FIRST 等）が信頼関係に基づき、攻撃手法、C2サーバーIP、対策情報を相互共有すること。
- **技術・運用ポイント**: TLP（Traffic Light Protocol: Red/Amber/Green/Clear）等の情報開示範囲ルールを適用して安全に共有する。
- **試験出題ポイント**: TLPによる開示範囲の制御とCSIRTアライアンス。
- **シラバス参照**: [1-7 関係者とのコミュニケーション](../syllabus_detail.md)
...
- **概要**: 組織内に設置される、セキュリティインシデント（ウイルス感染、情報漏洩、DOS攻撃等）発生時の緊急対応、被害局限化、調査、再発防止指示を専門に行う組織・チーム。
- **技術・運用ポイント**: トリアージ（優先度判断）、フォレンジック指示、経営陣やJPCERT/CCとの連絡窓口機能を担う。
- **試験出題ポイント**: SOC（ログ常時監視・一次検出）と CSIRT（事故発生時の対処・意思決定・復旧指揮）の役割の違い。
...
- **概要**: 収集した電子証拠が、いつ、誰によって、どのように取得・保管・移送され、改ざんされていないかを連続して証明する管理記録（証拠保管の連鎖）。
- **技術・運用ポイント**: 証拠採取時点と分析時点のハッシュ値（SHA-256等）の一致を証明する。
- **試験出題ポイント**: 法的証拠能力（Chain of Custody）を担保するためのハッシュ値管理と作業ログの保持。
- **シラバス参照**: [4-4 証拠の収集及び分析](../syllabus_detail.md)

#### <a id="揮発性順序(order-of-volatility)"></a>揮発性順序(Order of Volatility)
- **概要**: フォレンジック調査で証拠データを収集する際、失われやすい（揮発性の高い）情報から順番に収集・保全する標準原則 (RFC 3227)。
- **技術・運用ポイント**: 収集順序：レジスタ・キャッシュ ➔ メモリ (RAM) ➔ ネットワーク状態・接続 ➔ ディスク・ストレージ ➔ バックアップメディア ➔ 物理配線・紙。
- **試験出題ポイント**: 揮発性順序（メモリ/通信状態がディスクより先）の正確な優先度並べ替え問題。
- **シラバス参照**: [4-4 証拠の収集及び分析](../syllabus_detail.md)
